...
# prepare file
var f=open("be_matter_clusters.h", "w")
def fprint(*l)
  f.write(l.concat(" "))
  f.write("\n")
end
...
var cl_id_name = {}
for cl_id:c
  cl_id_name[cl_id['clusterId']] = cl_id['clusterName']
end
...
for clid:cl_ids
  ...
  var attributes = c[cl_id_name[clid]]['attributes']
  var attr_id_name = {}
  for attr:attributes
  ...
  fprint()
  # commands
  # fprint(f"const matter_command_t matter_Commands_{clid:04X}[] = {")
  var commands = c[cl_id_name[clid]]['commands']
  var cmd_id_name = {}
  for cmd:commands
...
for clusr:cl_ids
  fprint(format('  { 0x%04X, "%s", matter_Attributes_%04X, matter_Commands_%04X },', clusr, cl_id_name[clusr], clusr, clusr))
end
