...
class Matter_Device
  ...
  static var VENDOR_ID = 0x1404       #0xFFF1
  static var PRODUCT_ID = 0x8001      #0x8000
  static var LIGHT_ID   = 0x1000      #0x8000
  static var FILENAME = "_matter_device.json"
  static var PASE_TIMEOUT = 10*60     # default open commissioning window (10 minutes)
  var matter_type                     # matter type device
  ...
  def save_param()
    ...
    try
      ...
      f.write(j)
      f.close()
      tasmota.log(format("MTR: =Saved_parameters%s", self.plugins_persist ? " and configuration" : ""), 2)
      return j
    except .. as e, m
  ...
  def load_param()
    ...
    # 打印日志
    tasmota.log(format("MTR_QR Start: %d %d " ,self.root_discriminator,self.root_passcode),2)
    # 原始随机获取 discriminator 和 root_passcode
    if self.root_discriminator == nil || self.root_passcode == nil
      var filename = "qr.txt" 
      try
        var f = open(filename)
        if (f == nil)
          tasmota.log("MTR: qr File not found", 2)
        else
          var s = f.read()
          f.close(f)
          import json
          var import_json = json.load(s)  
          self.root_discriminator = import_json.find("discriminator",self.root_discriminator)
          self.root_passcode = import_json.find("passcode",self.root_passcode)
          dirty = true
          tasmota.log(format("MTR:qrCode: %d %d " ,self.root_discriminator,self.root_passcode),2)
        end
      except .. as e, m
          tasmota.log("load_param Exception:" + str(e) + "|" + str(m), 2)
      end
      if self.root_discriminator == nil || self.root_passcode == nil
    ...
    tasmota.log(format("MTR_QR End:%d %d " ,self.root_discriminator,self.root_passcode),2)
  ...
  def _instantiate_plugins_from_config(config)
    ...
    for ep: endpoints
      ...
      try
        ...
        tasmota.log(format("MTR:   endpoint = %5i %d type:%s-%s", ep,self.productid, pi_class_name, self.conf_to_log(plugin_conf)), 2)
        import string
        if (string.find(pi_class_name,"light") >= 0) 
          self.productid = self.LIGHT_ID
        end
        tasmota.log(format("MTR:productid =  %d",self.productid), 2)
      except .. as e, m
