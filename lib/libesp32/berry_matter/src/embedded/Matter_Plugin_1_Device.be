...
class Matter_Plugin_Device : Matter_Plugin
  ...
  def read_attribute(session, ctx, tlv_solo)
    ...
    # ====================================================================================================
      ...
      import string
      # tasmota.log(format("MTR_cluster:%x %d  %d " ,cluster,attribute,self.device.productid),2)
      if attribute == 0x0001          #  ---------- VendorName / string ----------
        if self.device.productid == 32768
          return tlv_solo.set(TLV.UTF1, "Tasmota")
        else
          return tlv_solo.set(TLV.UTF1, "Phaten")
        end
      elif attribute == 0x0003          #  ---------- ProductName / string ----------
        if self.device.productid == 32768
          return tlv_solo.set(TLV.UTF1, tasmota.cmd("DeviceName", true)['DeviceName'])
        else
          if self.device.productid == 32769
            return tlv_solo.set(TLV.UTF1, "Onoff")
          else
            return tlv_solo.set(TLV.UTF1, "lighting")
          end
        end
        ...
        if self.device.productid == 32768
          ...
          return tlv_solo.set(TLV.UTF1, "1.0")
        end
