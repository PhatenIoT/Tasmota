...
void UfsInit(void) {
  ...
  AddLog(LOG_LEVEL_INFO, PSTR("firmware buildTimes %s-%s"),__DATE__ ,__TIME__);
}
...
bool TfsLoadFile(const char *fname, uint8_t *buf, uint32_t len) {
  if (!ffs_type) { 
    AddLog(LOG_LEVEL_INFO, PSTR("TFS_Type %d"), ffs_type);
    return false; 
  }

  File file = ffsp->open(fname, "r");
  if (!file) {
    AddLog(LOG_LEVEL_INFO, PSTR("TFS_File '%s' not found"), fname +1);  // Skip leading slash
    return false;
  }
...
bool TfsDeleteFile(const char *fname) {
  ...
  if (!ffsp->remove(fname)) {
    AddLog(LOG_LEVEL_INFO, PSTR("TFS: Delete failed %s "),fname);
    return false;
  }
  return true;
}
...
bool TfsRenameFile(const char *fname1, const char *fname2) {
  ...
  if (!ffsp->rename(fname1, fname2)) {
    AddLog(LOG_LEVEL_INFO, PSTR("TFS: Rename failed %s %s "),fname1,fname2);
    return false;
  }
  return true;
}
