...
void SettingsLoad(void) {
  ...
  AddLog(LOG_LEVEL_INFO, PSTR("SettingsLoad: %d %d %d %d %d \n"), source,settings_location,Settings->cfg_holder,Settings->save_flag,Settings->module);
  if (source) {
    settings_location = 1;
...
//  if ((0 == settings_location) || (Settings->cfg_size != sizeof(TSettings)) || (Settings->cfg_holder != (uint16_t)CFG_HOLDER)) {  // Init defaults if cfg_holder differs from user settings in my_user_config.h
  ...
  AddLog(LOG_LEVEL_INFO, PSTR("SettingsLoad end %d "), Settings->module);
  if(Settings->module == 0)
  {
	SettingsDefault();
  }
...
void SettingsDefault(void) {
  ...
  SettingsDefaultSet3();
  SettingsSave(2);
  TfsDeleteFile("/._matter_device.json");
}
...
void SettingsDefaultSet3(void) {
   // user load file config 
  char* usrdata=NULL;
  char* templateStart=NULL;
  char* backlogStart=NULL;
  char* rule1Start=NULL;
  char* token = NULL;
  uint32_t data_len = 0;
  unsigned char loadfile = 0;
  // read UserConifg
  usrdata = (char*)malloc(1024 * sizeof(char));
  if (usrdata == NULL)
  {
    loadfile = 0;
  }
  else
  {
    ...
    {
        loadfile = 0;
        AddLog(LOG_LEVEL_INFO, "MTR_File not found");
    } 
    else 
    {
        size_t filelen = fread(usrdata, 1, 1024, fd); 
        loadfile = 1;
        token = strstr(usrdata, "Template ");
        templateStart = strtok(token, "\r\n");
        templateStart += strlen("Template ");

        token = strstr(usrdata, "BACKLOG ");
        backlogStart = strtok(token, "\r\n");
        backlogStart += strlen("BACKLOG ");

        token = strstr(usrdata, "RULE1 ");
        rule1Start = strtok(token, "\r\n");
        rule1Start += strlen("RULE1 ");
        if (templateStart && *(templateStart) == '{')
        {
            String user_template = templateStart;
            JsonTemplate((char*)user_template.c_str());
            //AddLog(LOG_LEVEL_INFO, PSTR("FILE_TEMPLATE %s %s "), user_template.c_str(),Settings->user_template);
        ...
        if (backlogStart)
        {
            ...
            user_backlog += backlogStart;
            ExecuteCommand((char*)user_backlog.c_str(), SRC_RESTART);
            //AddLog(LOG_LEVEL_INFO, PSTR("FILE_BACKLOG %s "), user_backlog.c_str());
        ...
        AddLog(LOG_LEVEL_INFO, PSTR("FILE_Load Succ %d "), loadfile);
        fclose(fd);  
    }
    free(usrdata);
  }
  ...
  if(loadfile == 0)
  {
        ...
        JsonTemplate((char*)user_template.c_str());
        AddLog(LOG_LEVEL_INFO, PSTR("USER_TEMPLATE %s %s "), user_template.c_str(),Settings->user_template);
        user_template = (const char*) nullptr;  // Force deallocation of the String internal memory
    #endif
        ...
        ExecuteCommand((char*)user_backlog.c_str(), SRC_RESTART);
        AddLog(LOG_LEVEL_INFO, PSTR("USER_BACKLOG %s "), user_backlog.c_str());
        user_backlog = (const char*) nullptr;   // Force deallocation of the String internal memory
    #endif
